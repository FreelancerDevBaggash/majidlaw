...
// جلب المقالات المنشورة مع دعم البحث، التصنيف، الوسوم
export async function getBlogPosts(
  page: number = 1,
  limit: number = 10,
  search: string = "",
...
// جلب مقالة واحدة بالمحتوى الكامل مع المقالات ذات الصلة
export async function getBlogPostBySlug(slug: string): Promise<BlogPostResponse> {
  await connectDB()

  const post = await BlogPost.findOne({ slug, published: true }).lean()
  if (!post) throw new Error("المقال غير موجود")
...
// جلب التصنيفات وعدد المقالات بكل تصنيف
export async function getBlogCategories(): Promise<BlogCategory[]> {
  await connectDB()
  const categories = await BlogPost.aggregate([
    { $match: { published: true } },
    { $group: { _id: "$category", count: { $sum: 1 } } },
    { $project: { name: "$_id", count: 1, slug: "$_id" } },
  ])
  return categories
}

// جلب المقالات الشائعة (الأحدث هنا كمثال)
export async function getPopularPosts(limit: number = 5): Promise<BlogPost[]> {
  await connectDB()
  return BlogPost.find({ published: true })
    .sort({ readTime: -1, createdAt: -1 })
    .limit(limit)
    .select("title slug image excerpt")
    .lean()
}
...
export async function getPostsByTag(tag: string, limit: number = 10): Promise<BlogPost[]> {
  await connectDB()
  return BlogPost.find({ published: true, tags: tag })
    .sort({ createdAt: -1 })
    .limit(limit)
    .select("title slug image excerpt")
    .lean()
...
// جلب جميع الوسوم مع عدد المقالات
export async function getAllTags(): Promise<{ name: string; count: number }[]> {
  await connectDB()
...
export async function searchBlogPosts(
  ...
  await connectDB()
  const skip = (page - 1) * limit
  const searchQuery = query ? { $text: { $search: query }, published: true } : { published: true }
...
export async function getSuggestedPosts(
  ...
  await connectDB()
  const query: any = { published: true, _id: { $ne: currentPostId } }
  if (category) query.category = category
  if (tags.length > 0) query.tags = { $in: tags.slice(0, 3) }
